...
- 본문 한국어. 전문용어는 첫 등장 시 `내적(inner product)` 병기, 이후 한국어.
...
- `main` 에 직접 push 하지 말라는 일반 규칙의 예외: **이 저장소는 main 직접 push 가 정상 워크플로다**
  (개인 학습 자료이고 리뷰어가 없다). 단 force-push 는 하지 않는다.
