...
# 8. 장을 하나 끝냈으면 표준 강의 목차와 대조한다 (SPEC §5.4-c)
#    칩 검사는 '항목이 빠졌는가'만 본다. 항목은 다 있는데 이해에 필요한
#    다리가 빠진 것 — 그림 없는 기하 서술, 이름 안 붙인 성질, 갈리는 용어,
#    구하는 법 없는 정의 — 은 목차 대조로만 걸린다.
#    커리큘럼에 없는 항목을 칩으로 추가하지는 않는다. 설명과 그림만 보탠다.

# 9. main 에 커밋 & 푸시 → 1분 뒤 자동 배포 (브랜치·PR 없음, 묻지 말고 바로)
...
- **정의·성질에는 `<span class="hl">` 형광펜, 핵심 수식에는 `.eq--key`** (SPEC §5.4-b).
  비유·예시는 칠하지 않는다. 한 개념에 한 문장.
