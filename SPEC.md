...
- 두 가지 대비를 함께 본다 — **글자 대비**(읽히는가)와 **종이 대비**(눈에 띄는가).
  ...
  | | 밴드 | 아래 테두리 | 글자 대비 | 종이 대비 |
  |---|---|---|---|---|
  | 라이트 | `rgb(235,180,36)` | `rgb(169,119,24)` | 9.42:1 | 1.79:1 (테두리 3.71:1) |
  | 다크 | `rgb(109,95,16)` | `rgb(161,137,10)` | 5.14:1 | 2.84:1 (테두리 5.26:1) |

  노랑은 크림색 종이와 밝기가 비슷해 종이 대비를 올리기 어렵다. 그래서
  **띠 아래쪽 8% 에 진한 테두리**(`--hl-edge`)를 깔아 경계를 만든다 —
  형광펜이 지나간 자국처럼 보이면서 종이와의 경계가 또렷해진다.
  순수 형광노랑(`#FFD600`)은 종이 대비가 1.2:1 뿐이라 쓰지 않는다.
