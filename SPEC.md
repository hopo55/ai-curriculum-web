...
- **띠는 처음부터 끝까지 한 가지 색이다.** 위아래 어디든 더 진한 구간을 두면
  그 자리가 선으로 읽혀 하이라이트에 밑줄이 그어진 것처럼 보인다.
  '형광펜 잉크가 고인 느낌'을 내려고 아래쪽을 진하게 하거나(`74%, 74%` 처럼)
  정지점을 겹쳐 쓰는 시도는 전부 밑줄로 보였다 — 하지 않는다.
  위아래 경계만 짧게 흐려(22→30%, 78→86%) 자국처럼 만든다.
