...
훑어보기만 해도 절의 뼈대가 보이도록 표시한다. **무엇을 칠하는가**가 기준의 전부다.

| 대상 | 무엇을 | 표시 |
|---|---|---|
| **정의** | 커리큘럼 항목이 처음 정의되는 문장 | `<span class="hl">` |
| **키워드** | 그 정의문 안의 용어 (따로 칠하지 않는다 — 정의문에 이미 들어 있다) | 〃 |
| **성질·조건** | 반드시 알아야 하는 판정 기준 (`det=0 이면 역행렬이 없다`) | 〃 |
| **핵심 수식** | 그 절의 뼈대가 되는 번호 붙은 식 한둘 | `<div class="eq eq--key">` |

- **칠하지 않는 것**: 비유·예시·역사 이야기, "이것만 기억하면" 식의 수사.
  그런 문장은 이미 `.tldr`·`.note` 가 맡고 있다.
- **한 개념에 한 문장.** 같은 개념을 여러 곳에서 칠하지 않는다.
- **`.note` · `.tldr` 안에는 칠하지 않는다.** 이미 강조된 자리라 표시가 두 겹이 된다.
- 절당 5~10개가 나온다 (커리큘럼 항목이 8~11개이므로 자연스럽게 그 범위가 된다).
- 정의가 `.note--key` 상자 안에 있는 개념은 상자가 그 역할을 하므로 건너뛴다.

```html
<span class="hl">길이가 1인 vector를 unit vector라 한다.</span>

<div class="eq eq--key">
  <div class="eq__body">$$ \mathbf{x}^\top\mathbf{y} = \|\mathbf{x}\|\,\|\mathbf{y}\|\cos\theta $$</div>
  <span class="eq__tag num">(2)</span>
</div>
```

#### 형광펜 표시 규칙 (CSS)

- **`<mark>` 을 쓰지 않는다.** 브라우저 기본 배경이 밝은 노랑이라, 스타일시트가 아직 안 왔거나
  캐시가 오래된 순간에 다크 모드에서 글자가 보이지 않는다. `<span class="hl">` 은 스타일이 없으면
  그냥 평범한 글자가 된다.
- **띠는 처음부터 끝까지 한 가지 색이다.** 위아래 어디든 더 진한 구간을 두면 그 자리가 선으로 읽혀
  밑줄이 그어진 것처럼 보인다. 정지점을 겹쳐 쓰거나(`74%, 74%`) 아래쪽을 진하게 하는 시도는
  전부 밑줄로 보였다 — 하지 않는다. 위아래 경계만 짧게 흐린다.
- **호박색(amber) 계열**을 쓴다. `.note--key`·`.eq--key` 와 같은 계열이라 "핵심"이라는 뜻이 일관된다.
  주묵(`--seal`)은 완료·채점에 남겨 둔다.
- 두 가지 대비를 함께 본다 — **글자 대비**(읽히는가)와 **종이 대비**(눈에 띄는가).
  종이 대비가 1.5:1 아래면 배경에 묻혀 칠한 의미가 없다.

  | | 띠 | 밴드 안 글자 | 글자 대비 | 종이 대비 |
  |---|---|---|---|---|
  | 라이트 | `rgb(224,160,19)` | 그대로(먹색) | 7.83:1 | 2.15:1 |
  | 다크 | `#ffd54f` | `#14161a` 로 뒤집음 | 12.84:1 | 12.84:1 |

- **라이트와 다크는 방식이 다르다.** 종이가 밝으면 밴드를 어둡게, 어두우면 밴드를 밝게 둔다.
  - 라이트: 종이가 크림색이라 밝은 노랑은 묻힌다(`#FFD600` 은 종이 대비 1.2:1).
    진한 황금노랑을 쓰고 글자색은 건드리지 않는다.
  - 다크: **밴드를 밝게 하고 밴드 안 글자를 어둡게 뒤집는다.** 종이(거의 검정)와
    글자(거의 흰색)가 양 끝에 있어, 흰 글자를 유지한 채로는
    **글자 대비와 종이 대비가 둘 다 3.82:1 을 못 넘는다**(둘을 같게 놓고 푼 상한).
    AA 4.5:1 에 못 미치므로 투명도를 올리거나 색을 바꾸는 것으로는 해결되지 않는다.
    글자색을 뒤집으면 두 대비가 동시에 12.84:1 이 된다.
  - 글자색을 뒤집을 때는 **띠가 글자를 완전히 덮어야 한다**.
    삐져나온 글자는 어두운 종이 위 어두운 글자가 되어 사라진다.
  - **띠 높이는 13%~88%.** 글자는 줄 상자의 대략 22%~78% 를 차지하므로
    위아래로 각각 2.7px · 3.0px 의 여백이 남는다(본문 17px · 행간 1.78 → 줄 상자 30.3px).
    이보다 좁히면 글자에 달라붙어 답답하고, 더 넓히면 윗줄·아랫줄 띠 사이 간격(4.2px)이
    사라져 문단 전체가 한 덩어리로 보인다.
  - `strong` 은 `color: var(--ink)` 를 갖고 있어 `.hl strong { color: inherit }` 로
    되돌리지 않으면 다크에서 굵은 글자만 밝게 남는다. `em`·`a` 도 같다.
- 수식 `$…$` 를 가운데서 자르지 않는다. 잘리면 KaTeX 가 렌더하지 못한다.

### 5.4-c 표준 강의와 목차를 대조한다

칩 검사(파트 1)는 **커리큘럼 항목이 빠졌는가**만 본다. 항목은 다 있는데
그 항목을 **이해시키는 데 필요한 다리**가 빠진 것은 잡지 못한다.
그래서 장을 마친 뒤 그 분야의 표준 강의 목차와 한 번 맞춰 본다.

- 선형대수 — Khan Academy *Linear Algebra* (144편), MIT 18.06, 3Blue1Brown *Essence of Linear Algebra*
- 미적분·최적화 — Khan Academy *Multivariable Calculus*, 3Blue1Brown *Essence of Calculus*
- 확률·통계 — Khan Academy *Statistics and Probability*

대조는 "그쪽에 있는 항목을 전부 넣는다"가 아니다. 커리큘럼에 없는 항목은
**칩을 늘리지 않는다.** 넣는 것은 이미 있는 항목을 떠받치는 설명·그림뿐이다.
2장을 대조했을 때 실제로 걸린 것은 네 가지 유형이었다.

| 유형 | 증상 | 2장에서 나온 예 |
|---|---|---|
| **글은 있는데 그림이 없다** | 본문이 "기하적으로는 …이다"로 한 줄 때우고 끝 | least squares = column space 위로의 정사영 |
| **사실은 있는데 이름이 없다** | 성질은 적어 놨는데 표준 명칭을 안 붙여, 다른 책에서 만나면 못 알아본다 | `Row ⊥ Null` 은 있는데 orthogonal complement 라는 이름이 없었다 |
| **같은 것을 다르게 부른다** | 교재·강의마다 용어가 갈리는데 대응표가 없다 | `im/ker` ↔ `Col/Null`, 외적 = outer product ≠ cross product |
| **결과만 있고 구하는 법이 없다** | 정의는 했는데 실제로 어떻게 뽑는지가 없다 | `Col(A)` 의 basis 를 RREF 의 pivot 열로 찾는 법 |

용어가 갈리는 곳에서는 **우리가 어느 쪽을 쓰는지 밝히고 대응표를 둔다.**
학습자가 다른 자료로 넘어갈 때 걸려 넘어지는 자리가 대부분 여기다.
...
> **이 검사는 위젯의 초기 상태만 본다.** 슬라이더를 끝까지 밀면 그림이 칸을 넘치는 경우는
> 잡히지 않는다. 슬라이더가 있는 위젯을 새로 만들면 **모든 슬라이더의 양 끝·중간 조합을
> 훑는 임시 스크립트를 따로 돌려서** 확인한다 (jsdom 으로 `input` 이벤트를 쏘고
> 매번 요소 좌표를 재면 된다). 확인이 끝나면 그 스크립트는 지운다.
